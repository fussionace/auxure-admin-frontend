...
import { Moon, Alert, AlertDark, NewMessage, ArrowDown, Sun, ArrowdownDark, Search, Chevrondown, ChevronLeft, ChevronRight,  } from '../assets/svgs/svg-icons'
import Ellipse from '../assets/images/Ellipse.png'

const Transaction = ({ toggleTheme, darkTheme }) => {
  const products = [
    {
      id: '#14327A',
    ...
    <div className='ml-[18.125rem] p-[2rem] bg-light-gray dark:bg-darkest min-h-screen'>
      ...
      <div className="form-filters flex justify-between items-center mb-[2rem]">
        <div className="flex-container flex items-center gap-[1.6rem]">
          <div className="form-field relative">
            <input type="text" className='text-[16px] font-light text-gray rounded-[.25rem] px-[1rem] py-[.5rem] pr-[2rem] outline-none' placeholder='Search...'/>
            <Search className='absolute top-[30%] right-[4%] cursor-pointer'/>
          </div>
          <div className="bg-white status h-[2.5rem] px-[1rem] py-[.5rem] rounded-[.25rem] flex items-center gap-[.375rem] justify-center cursor-default">
              <span className="block text-[.9375rem] font-medium text-gray">Status : All</span>
              <Chevrondown /> 
          </div>
        </div>
        <div className="bg-white status h-[2.5rem] px-[1rem] py-[.5rem] rounded-[.25rem] flex items-center gap-[.375rem] justify-center cursor-default">
          <span className="block text-[.9375rem] font-medium text-gray">Filter by date range</span>
          <Chevrondown /> 
        </div>
      </div>
      ...
      <div className='bg-white dark:bg-dark-primary rounded-[.625rem]'>
        <table className='w-full dark:text-white'>
          <thead>
            <tr className='border-b-2 border-light-gray text-left text-[.8125rem] font-semibold text-gray'>
          ...
          <tbody>
            {
              values.map((order) => (
                <tr className='last:pl-[2rem] border-b-2 border-light-gray' key={order.id}>
                  <td className='px-[2rem] text-[.875rem] font-light h-[3.3125rem]'>{ order.id }</td>
                  ...
                  <td className='text-[.875rem] font-light'>{ order.paymentMethod }</td>
                  <td className={order.status ===  'Canceled' ? 'text-[.875rem] font-light text-canceled' : order.status === 'Delivered' ? 'text-8[.75rem] font-light text-delivered' : ''}>{ order.status }</td>
        ...
        <div className="pagination-container flex items-center justify-between mt-[1rem] pb-[2rem] px-[2rem]">
          <div className="numbers-flex flex items-center gap-[.6rem]">
            <label htmlFor="number" className='text-[.9375rem] font-semibold dark:text-gray'>Showing</label>
          ...
          <div className="pagination flex items-center justify-center gap-[.25rem]">
            ...
            {
              btnCount.map((btn) => 
                  <button key={Math.random()} className={btn === activePage ? 'bg-black text-white w-[1.75rem] h-[1.75rem] text-[.8125rem] font-light rounded-[.25rem]': 'rounded-[.25rem] text-gray bg-light-gray w-[1.75rem] h-[1.75rem] border-light-gray border-solid border-[1px] text-[.8125rem]'} onClick={ () =>  handleChange(btn) }>
                      { btn }
                  </button>
              )
            }
            <button className='w-[1.75rem] h-[1.75rem] bg-light-gray flex items-center justify-center text-base border-light-gray border-solid border-[1px] rounded-[.25rem]' onClick={ onNextClick } disabled={ activePage === btnCount.length }>
              <ChevronRight />
            </button>
          </div>  
        </div>
